<template>
    <el-table :data="getAllCountries" :default-sort = "{ prop: 'name', order: 'ascending' }" style="width: 100%" stripe>
        <el-table-column label="Code" prop="code" width="90px" sortable>
            <template slot-scope="scope">
                <div v-if="getIsRowInEditMode(scope)">
                    <el-input placeholder="Code" v-model="codeInput"></el-input>
                </div>
        ...
        <el-table-column label="Name" prop="name" sortable>
            <template slot-scope="scope">
                <div v-if="getIsRowInEditMode(scope)">
                    <el-input placeholder="Country name" v-model="nameInput"></el-input>
                </div>
        ...
        <el-table-column label="Operations" align="right" width="200px">
            <template slot-scope="scope">
                <el-button :class="{ 'el-button--primary' : getIsRowInEditMode(scope) }" size="mini" @click="onClickEdit(scope.row)">
      ...
      methods: {
        ...
        onClickEdit({ code, name }) {
          ...
          if (!this.editingCountryCode) {
            // submitting edit
            /* eslint-disable */ console.log(`Test - SUBMITTING edit for country: ${name}`);
            return;
          }
          this.codeInput = code;
          this.nameInput = name;
